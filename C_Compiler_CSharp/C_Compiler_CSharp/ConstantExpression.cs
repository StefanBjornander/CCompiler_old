...
namespace CCompiler {
  public class ConstantExpression {
    public static bool IsConstant(Expression expression) {
      ...
      return (type.IsLogical() && ((symbol.TrueSet.Count == 0) || symbol.FalseSet.Count == 0)) ||
    ...
    public static bool IsTrue(Expression expression) {
      return ((expression.Symbol.Value is BigInteger) && !((BigInteger) expression.Symbol.Value).IsZero) ||
             ((expression.Symbol.Value is decimal) && !((decimal) expression.Symbol.Value).Equals((decimal) 0)) ||
             ((expression.Symbol.TrueSet != null) && (expression.Symbol.TrueSet.Count > 0));
    }
    ...
    public static Expression Relation(MiddleOperator middleOp,
      ...
      else if (leftExpression.Symbol.Type.IsFloating() || rightExpression.Symbol.Type.IsFloating()) {
    ...
    private static Expression RelationIntegral(MiddleOperator middleOp,
      ...
      Symbol symbol = resultValue ? (new Symbol(null, jumpSet)) : (new Symbol(jumpSet, null));
    ...
    private static Expression ArithmeticIntegral(MiddleOperator middleOp,
      ...
      Symbol symbol = ArithmeticIntegral(middleOp, leftExpression.Symbol, rightExpression.Symbol);
    ...
    public static Symbol ArithmeticIntegral(MiddleOperator middleOp,
      ...
      switch (middleOp) {
        case MiddleOperator.BinaryAdd:
          if (leftType.IsPointerOrArray()) {
            resultValue = leftValue + (rightValue * leftType.PointerOrArrayType.Size());
          }
          else if (leftType.IsPointerOrArray()) {
            resultValue = (leftValue * rightType.PointerOrArrayType.Size()) + rightValue;
          }
          else {
            resultValue = leftValue + rightValue;
          }
      ...
      resultSymbol.StaticSymbol = Value(resultSymbol.UniqueName, maxType, resultValue);
      SymbolTable.StaticSet.Add(resultSymbol.StaticSymbol);
      return resultSymbol;*/
    }
    ...
    private static Expression ArithmeticFloating(MiddleOperator middleOp,
      ...
      Type maxType = TypeCast.MaxType(leftExpression.Symbol.Type, rightExpression.Symbol.Type);
      Symbol resultSymbol = new Symbol(maxType, resultValue);
      //resultSymbol.StaticSymbol = Value(resultSymbol.UniqueName, maxType, resultValue);
      //SymbolTable.StaticSet.Add(resultSymbol.StaticSymbol);
      List<MiddleCode> codeList = new List<MiddleCode>();
      MiddleCodeGenerator.AddMiddleCode(codeList, MiddleOperator.PushFloat, resultSymbol);
    ...
    public static Expression LogicalNot(Expression expression) {
      if (IsConstant(expression)) {
        ...
        Symbol symbol = isTrue ? (new Symbol(null, jumpSet)) : (new Symbol(jumpSet, null));
        return (new Expression(symbol, null, codeList));
      }
    ...
    public static Expression Arithmetic(MiddleOperator middleOp, Expression expression) {
      if (!IsConstant(expression)) {
        return null;
      }
    ...
    private static Expression ArithmeticIntegral(MiddleOperator middleOp, Expression expression) {
      ...
      //resultSymbol.StaticSymbol = Value(resultSymbol.UniqueName, expression.Symbol.Type, resultSymbol.Value);
      //SymbolTable.StaticSet.Add(resultSymbol.StaticSymbol);
      return (new Expression(resultSymbol, null, null));
    }

    private static Expression ArithmeticFloating(MiddleOperator middleOp, Expression expression) {
    ...
    public static Expression Cast(Expression fromExpression, Type toType) {
      ...
      if (fromType.Equals(toType)) {
      ...
      else if (((fromType.IsIntegralArrayOrPointer() && toType.IsIntegralArrayOrPointer()) ||
                (fromType.IsFloating() && toType.IsFloating())) && (fromType.Size() == toType.Size())) {
        Symbol toSymbol = new Symbol(toType, fromValue);
        return (new Expression(toSymbol, fromExpression.ShortList, fromExpression.LongList));
      }
      else if (fromType.IsLogical() && toType.IsIntegralArrayOrPointer()) {
        if ((fromSymbol.TrueSet.Count > 0) && (fromSymbol.FalseSet.Count == 0)) {
          Symbol toSymbol = new Symbol(toType, (BigInteger) 1);
          return (new Expression(toSymbol, null, null));
        }
        else if ((fromSymbol.TrueSet.Count == 0) && (fromSymbol.FalseSet.Count > 0)) {
          Symbol toSymbol = new Symbol(toType, (BigInteger) 0);
          return (new Expression(toSymbol, null, null));
        }
      }
      else if (((fromType.IsIntegralArrayOrPointer() || (fromValue is BigInteger)) ||
                (fromType.IsFloating() && (fromValue is decimal))) && toType.IsLogical()) {
        List<MiddleCode> codeList = new List<MiddleCode>();
        MiddleCode jumpCode = new MiddleCode(MiddleOperator.Goto);
        codeList.Add(jumpCode);
        ISet<MiddleCode> jumpSet = new HashSet<MiddleCode>();
        ...
        Symbol toSymbol;
        if (fromValue.Equals(BigInteger.Zero) || fromValue.Equals((decimal) 0)) {
        ...
        if (fromType.IsIntegralArrayOrPointer() && toType.IsIntegralArrayOrPointer()) {
        ...
        if (toType.IsFloating()) {
          MiddleCodeGenerator.AddMiddleCode(codeList, MiddleOperator.PushFloat, toSymbol);
        }

        //toSymbol.StaticSymbol = Value(toSymbol.UniqueName, toType, toSymbol.Value);
        //SymbolTable.StaticSet.Add(toSymbol.StaticSymbol);
        return (new Expression(toSymbol, null, codeList));
      }
    ...
    public static StaticSymbol Value(string uniqueName, Type type, object value) {
      List<MiddleCode> middleCodeList = new List<MiddleCode>();
      
      if (value != null) {
        middleCodeList.Add(new MiddleCode(MiddleOperator.Init, type.Sort, value));
      }
      else {
        middleCodeList.Add(new MiddleCode(MiddleOperator.InitZero, type.Size()));
      }
      ...
      if (Start.Windows) {
        ...
        IDictionary<int,string> accessMap = new Dictionary<int,string>(); //ListMap
        //GenerateStaticInitializerWindows.ByteList(assemblyCodeList, byteList, accessMap);
      ...
      if (Start.Linux) {
        ...
        return (new StaticSymbolLinux(StaticSymbolLinux.TextOrData.Data, uniqueName, textList, externSet));
      }
