    ...
    public static List<MiddleCode> GenerateStatic(Type toType, object fromInit) {      
      ...
      if (fromInit is Expression) {
        ...
        if (toType.IsArray() && fromType.IsString()) {
          ...
          middleCodeList.Add(new MiddleCode(MiddleOperator.Init, fromSymbol.Type.Sort, fromSymbol.Value));
        }
          ...
          middleCodeList.Add(new MiddleCode(MiddleOperator.Init, toType.Sort, fromSymbol.Value));
        }
          ...
          if (fromType.IsArray()) {
            StaticAddress staticAddress = new StaticAddress(fromSymbol.UniqueName, 0);
            middleCodeList.Add(new MiddleCode(MiddleOperator.Init, toType.Sort, staticAddress));
          }
          else {
            middleCodeList.Add(new MiddleCode(MiddleOperator.Init, toType.Sort, fromSymbol.Value));
          }
          ...
          Expression toExpression = TypeCast.ImplicitCast(fromExpression, toType);
          Symbol toSymbol = toExpression.Symbol;
          Assert.Error(toSymbol.Value != null, toSymbol, Message.Non__constant_expression);
          middleCodeList.Add(new MiddleCode(MiddleOperator.Init, fromSymbol.Type.Sort, fromSymbol.Value));
        }
      }
      else {
        ...
        switch (toType.Sort) {
          case Sort.Array: {
              ...
              foreach (object value in fromList) {
                middleCodeList.AddRange(GenerateStatic(toType.ArrayType, value));
              }

              int restSize = toType.Size() - (fromList.Count * toType.ArrayType.Size());
              middleCodeList.Add(new MiddleCode(MiddleOperator.InitZero, restSize));
            }
            break;
          ...
          case Sort.Struct: {
              ...
              for (int index = 0; index < fromList.Count; ++index) {
                ...
                middleCodeList.AddRange(GenerateStatic(memberSymbol.Type, init));
                size += memberSymbol.Type.Size();
              }

              middleCodeList.Add(new MiddleCode(MiddleOperator.InitZero, toType.Size() - size));
            }
            break;
          ...
          case Sort.Union: {
              ...
              middleCodeList.Add(new MiddleCode(MiddleOperator.InitZero, toType.Size() - firstSymbol.Type.Size()));
            }
            break;
