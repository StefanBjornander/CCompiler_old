                                                  ...
                                                  object fromInitializer)
    {
      ...
      if (fromInitializer is Expression)
      {
        ...
        else if (toType.IsPointer() && fromType.IsString())
        {
          ...
          middleCodeList.Add(new MiddleCode(MiddleOperator.Initializer,
                  Sort.Pointer, Symbol.ValueName(toType, fromSymbol.Value)));
        }
          ...
          case Sort.Array:
            {
              ...
              middleCodeList.Add(new MiddleCode(MiddleOperator.
                                                InitializerZero, restSize));
            }
            break;
          ...
          case Sort.Struct:
            {
              ...
              for (int index = 0; index < fromList.Count; ++index)
              {
                ...
                object initializer =
                  ModifyInitializer.DoInitializer(memberSymbol.Type,
                                                  fromList[index]);
              ...
              middleCodeList.Add(new MiddleCode(MiddleOperator.
                                      InitializerZero, toType.Size() - size));
            }
            break;

          case Sort.Union:
            {
              Assert.Error(fromList.Count == 1, toType, Message.
                         A_union_can_be_initializeralized_by_one_value_only);
              IDictionary<string, Symbol> memberMap = toType.MemberMap;
              Symbol firstSymbol = memberMap.Values.GetEnumerator().Current;
              object initializer =
                ModifyInitializer.DoInitializer(firstSymbol.Type,
                                                fromList[0]);
              middleCodeList.AddRange(GenerateStatic(firstSymbol.Type,
                                                     initializer));
              middleCodeList.Add(new MiddleCode(MiddleOperator.
                                                InitializerZero, toType.Size()
                                                - firstSymbol.Type.Size()));
            }
            break;
